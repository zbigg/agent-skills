...
- **Patterns render on polygon tilesets only.** `h3` / `quadbin` accept the same fields (the schema keeps them for forward-compatibility) but nothing draws them today, so a patterned h3 layer silently renders flat.
